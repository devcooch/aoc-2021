...
fn get_operand(rhs: &str) -> Operand {
    let num_parse_res = rhs.parse::<isize>();
    if num_parse_res.is_ok() {
        Operand {
            typ: RHType::Num,
            value: num_parse_res.unwrap(),
            reg: 0,
        }
    } else {
        Operand {
    ...
    for abcd in all_abcd {
        for digit in (1..=9).rev() {
            execute_optimized(
                &abcd,
                digit,
                &mut x[(digit - 1) as usize],
                &mut z[(digit - 1) as usize],
            );
        }
    }
    /*
    let abcd = all_abcd.iter().nth(0).unwrap();
    for z_last in (0..=0).rev() {
        for digit in (1..=9).rev() {
            let mut x = 0;
            let mut z = z_last;
            execute_optimized(&abcd, digit, &mut x, &mut z);
            println!("Num {}, X={}, Z={}",
                z_last, x, z);
        }
    }
